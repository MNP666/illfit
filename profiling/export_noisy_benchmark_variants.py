...
DEFAULT_CONFIG_PATH = ROOT / "profiling" / "noisy_benchmark.toml"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export deterministic noisy observed-I(q) variants from a committed regression suite."
    )
    parser.add_argument(
        "--config",
        type=Path,
...
def load_config(path: Path) -> NoisySuiteConfig:
    ...
    return NoisySuiteConfig(
        ...
        scale_reference=str(noise.get("scale_reference", "max_intensity")),
    )
...
def write_noisy_iq_csv(
    ...
    with path.open("w", newline="", encoding="utf-8") as handle:
        ...
        writer.writerow(["q", "i_of_q_truth", "i_of_q_observed", "noise"])
        for q_value, truth, observed, delta in zip(q_values, truth_iq, observed_iq, noise, strict=True):
            writer.writerow(
                [
                    f"{q_value:.12g}",
                    f"{truth:.12g}",
...
def main() -> int:
    ...
    for level_index, noise_level in enumerate(config.levels):
        ...
        for row in accepted_summary:
            ...
            if config.scale_reference == "max_intensity":
                noise_scale = noise_level * float(np.max(truth_iq))
            else:
            ...
            noise = rng.normal(loc=0.0, scale=noise_scale, size=truth_iq.shape)
